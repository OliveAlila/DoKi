...
const envSchema = z.object({
  ...
  SIMULATE_AI: z.preprocess((val) => val === 'true' || val === true, z.boolean()).default(false),
  GEMINI_API_KEY: z.string().optional(),
}).refine(data => data.SIMULATE_AI || !!data.GEMINI_API_KEY, {
  message: "FATAL: GEMINI_API_KEY environment variable is missing, and SIMULATE_AI is false.",
  path: ["GEMINI_API_KEY"]
});
