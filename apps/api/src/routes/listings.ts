...
const ai =
	env.GEMINI_API_KEY && !env.SIMULATE_AI
...
const classifySchema = z.object({
	image: z
		.string()
		.min(1, "Visual verification stream (image payload) is required"),
});
...
const publishListingSchema = z.object({
	categoryId: z
		...
		.transform((val) => {
			...
			if (Number.isNaN(parsed))
				throw new Error("Category ID must be a valid number reference");
			return parsed;
		})
	...
	quantity: z
		...
		.transform((val) => {
			const parsed = Number(val);
			if (Number.isNaN(parsed))
				throw new Error("Feedstock volume quantity must be a valid number");
			return parsed;
		})
		.refine(
			(val) => val > 0,
			"Feedstock volume quantity must be positive and greater than zero (no negative weights)",
		),
	moisture: z
		.union([z.number(), z.string()])
		.transform((val) => {
			const parsed = Number(val);
			if (Number.isNaN(parsed))
				throw new Error("Relative Moisture Coefficient must be a valid number");
			return parsed;
		})
		.refine(
			(val) => val >= 0 && val <= 100,
			"Relative Moisture Coefficient (RMC) must be between 0 and 100 percent",
		),
	purity: z
		.union([z.number(), z.string()])
		.transform((val) => {
			...
			if (Number.isNaN(parsed))
				throw new Error(
					"Verified Composition Purity Index must be a valid number",
				);
			return parsed;
		})
		.refine(
			(val) => val >= 0 && val <= 100,
			"Verified Composition Purity Index (CPI) must be between 0 and 100 percent",
		),
...
const expressInterestSchema = z.object({
	listingId: z
		...
		.transform((val) => {
			const parsed = Number(val);
			if (Number.isNaN(parsed))
				throw new Error("Listing ID must be a valid number reference");
			return parsed;
		})
		...
		try {
			...
			if (categories.length === 0) {
				return res.status(500).json({
					error: "No categories found in database. Please run seed script.",
				});
			}
			...
			if (env.SIMULATE_AI) {
				...
				const confidence = parseFloat((0.8 + (dataLen % 20) / 100).toFixed(2));
				const moisture = parseFloat((15.0 + (dataLen % 45)).toFixed(1));
				const purity = parseFloat((70.0 + (dataLen % 30)).toFixed(1));
				const flaggedContaminants =
					dataLen % 7 === 0 ? ["plastic_contamination"] : [];
				...
				if (!ai) {
					return res.status(500).json({
						error:
							"AI Simulation is disabled, but no GEMINI_API_KEY is configured.",
					});
				}
				...
				try {
					...
					console.warn(
						"JSON parsing of Gemini response failed, applying robust fallback/regex:",
						parseError,
					);
					...
					let parsedConfidence = confMatch?.[1]
					...
					let parsedMoisture = moistMatch?.[1]
						? parseFloat(moistMatch[1])
						: 45.0;
					if (
					...
					let parsedPurity = purityMatch?.[1]
						? parseFloat(purityMatch[1])
						: 95.0;
					if (
						Number.isNaN(parsedPurity) ||
						parsedPurity < 0 ||
						parsedPurity > 100
					) {
						parsedPurity = 95.0;
					...
					if (catMatch?.[1]) {
						...
						const matchesAnyDb = categories.find(
							(c) => c.name.toLowerCase() === matchedText.toLowerCase(),
						);
						if (matchesAnyDb) {
			...
			let matchedCategory = categories.find(
				(c) => c.name.toLowerCase() === resultJson.categoryName?.toLowerCase(),
			);
			if (!matchedCategory) {
			...
			res.json({
				...
				is_manually_corrected:
					isManuallyCorrected || resultJson.is_manually_corrected || false,
			});
		} catch (error) {
...
router.post("/listings", authenticateJWT, async (req: AuthRequest, res) => {
	...
	if (!sellerId) {
		return res
			.status(401)
			.json({ error: "Unauthorized: Missing operator profile" });
	}
	...
	const { categoryId, quantity, moisture, purity, is_manually_corrected } =
		result.data;
	...
	try {
		...
		res
			.status(201)
			.json({ message: "Listing published successfully", listing });
	} catch (error) {
		console.error("Publish error:", error);
		res
			.status(500)
			.json({ error: "Internal server error while publishing listing" });
	}
});
...
router.get(
	...
	async (req: AuthRequest, res) => {
		...
		try {
			...
			const totalWeightKg = transactions.reduce(
			...
			res
				.status(500)
				.json({ error: "Internal server error loading dashboard stats" });
		}
	},
);
...
router.get("/transactions", authenticateJWT, async (req: AuthRequest, res) => {
	...
	try {
		...
		res
			.status(500)
			.json({ error: "Internal server error fetching transactions" });
	}
});
...
router.post(
	...
	async (req: AuthRequest, res) => {
		...
		if (role !== "BUYER") {
			return res
				.status(403)
				.json({ error: "Only buyers can express interest" });
		}
		...
		try {
			...
			res
				.status(500)
				.json({ error: "Internal server error while expressing interest" });
		}
	},
);
...
router.get(
	...
	async (req: AuthRequest, res) => {
		...
		if (role !== "BUYER") {
			return res
				.status(403)
				.json({ error: "Forbidden: Buyer clearance required" });
		}
		...
		try {
			...
			res
				.status(500)
				.json({ error: "Internal server error fetching buyer transactions" });
		}
	},
);
